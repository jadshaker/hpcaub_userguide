...
.. todo:: embed the SSH-keys screencast here once it is uploaded. It walks through the
    whole workflow end to end: generating an ed25519 key pair, copying the public key to
    the cluster with ``ssh-copy-id`` and logging in with the key.
...
.. todo:: embed the SSH-tunnel screencast here once it is uploaded. It walks through a
    local port forward end to end: serving a page on the head node, failing to reach it
    from your machine, opening the tunnel with ``ssh -L`` and reaching the page through it.
